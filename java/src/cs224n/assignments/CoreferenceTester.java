...
public class CoreferenceTester<SYS extends CoreferenceSystem> {
  ...
  public static String dataPath = "/afs/ir/class/cs224n/data/pa3/";
...
//      if(Math.abs(recall-check) > 1e-6){ throw new IllegalStateException("MUC scores do not agree (not your fault)!"); }
  ...
  public String debug(File[] data, Properties props){
    ...
    for(File f : data){
      ...
      debug.append("=====Document " + doc.id.replaceAll("/",".") +
		   "=====\n").append("These are your versions of the gold" 
				     + "clusters, with same-colored mentions indicating the proper gold clusters for improperly"
				     + "clustered words and black mentions currently missing from the"
				     + "cluster:")
	  .append("\n").append(doc.debug(guess,gold)).append("\n")
	  .append("These are the gold clusters, with same-colored mentions indicating your clusters:")
	  .append("\n");
      //Now we try to print out guess and gold in reverse - we
      // need to obtain the list of entities corresponding to the guess clusters
      // and the list of clustered mentions corresponding to the gold clusters
      Collection<Entity> guessEntities = Entity.fromMentions(guess);
      HashSet<ClusteredMention> goldClusters = new HashSet<ClusteredMention>();
      for(Entity e: gold){
	  Entity newE = new Entity(e);
	  Set<Mention> temp = new HashSet<Mention>(e.mentions);
	  for(Mention m: temp){
	      e.remove(m);
	      m.removeCoreference();
	      goldClusters.add(m.markCoreferent(newE));
	  }       
      }
      Document docRev = datum.document;
      debug.append(docRev.debug(goldClusters, guessEntities)); 
      Collection<Entity> goldEntities = new HashSet<Entity>();
      for (ClusteredMention cm : goldClusters) {
	  goldEntities.add(cm.entity);
      }
      debug.append("\nHere is the entire document; same-colored mention text indicates the gold clusters, while " +
		   "same-colored braces indicate your current clusters.").append("\n");
      debug.append(doc.prettyPrint(goldEntities, guessEntities)).append("\n");
      if (guessEntities.size() > 20) {
	  debug.append("**Caution: your guess contains more than 20"
		       + "unique entities, so some colors/styles may be" 
		       + "repeated due to ANSI limitations.\n");
      }
      debug.append("\n\n");
    }
    //--Return
  ...
  public static void main(String[] args) {
    ...
    //(get path)
    dataPath = props.getProperty("path","/afs/ir/class/cs224n/data/pa3/");
