...
public class Document implements Serializable, Decodable {
  ...
  public String prettyPrint(Collection<Entity> entities){
    ...
    // Creates a formatted string for the document, marking mentions with the same color and
    // marking guesses with the same color of brackets
    public String prettyPrint(Collection<Entity> gold,
			      Collection<Entity> guess) {
	//--Variables
	int sentenceIndex = 0;
	StringBuilder b = new StringBuilder();
	//--Print Sentences
	for(Sentence s : sentences){
	    b.append("(").append(sentenceIndex).append(") ");
	    s.prettyPrint(b,getMentions(),gold,guess);
	    if(sentenceIndex < sentences.size()-1){ b.append("\n"); }
	    sentenceIndex += 1;
	}
	return b.toString();
