 ...
 * 
 * Modified by Chris Billovits (chrisb at cs.stanford)
 */
public class CoreNLPDocument implements Serializable {
  ...
  public final CollectionValuedMap<String,CoreMap> corefChainMap;
    ...
    /* get[Field]ByReflection
     * The CoreNLP documentation for a Conll document contains static (package-visible)
     * variables that are set and used within the package, but without
     * public accessors.
     *
     * Later versions of CoreNLP are not updated with accessors.  A custom precompiled
     * CoreNLP jar is provided in the compile path.  Given this legacy
     * code, using reflection here is the best we can do (until someone
     * fixes this in CoreNLP!) This specific instance of reflection is
     * safe; i.e. the given document will always have the fieldName.
     */    
    
    private String getDocumentIdPartByReflection(){
	try {
	    Field f = this.doc.conllDoc.getClass().getDeclaredField("documentIdPart");
	    f.setAccessible(true);
	    return (String) f.get(this.doc.conllDoc);
   ...
   /* Because of type erasure, this is technically an unsafe
    * operation; on this corenlp jar it is guaranteed to work
    * with the conll document class as given.
    */
    @SuppressWarnings("unchecked")
    private List<CoreMap> getNerChunksByReflection(){
	try {
	    Field f = this.doc.conllDoc.getClass().getDeclaredField("nerChunks");
	    f.setAccessible(true);
	    
	    return (List<CoreMap>) f.get(this.doc.conllDoc);
  ...
  public CoreNLPDocument(edu.stanford.nlp.dcoref.Document doc){
    ...
    this.documentIdPart = getDocumentIdPartByReflection();
    this.nerChunks = getNerChunksByReflection();
