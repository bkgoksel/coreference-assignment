...
public class Sentence implements Serializable, Decodable {
  ...
  public StringBuilder prettyPrint(StringBuilder b, List<Mention> mentions, Collection<Entity> entities) {
    ...
    /**
     * Print this sentence nicely, highlighting both gold and guessed
     * mentions
     * @param b The builder to append to
     * @param mentions The mentions that might occur in the sentence
     * @param entities The gold entities to be printed
     * @param guess The guessed entities to be printed
     * @return The string builder passed as an argument with the sentence
     * appended
     */
    public StringBuilder prettyPrint(StringBuilder b, List<Mention>
				     mentions, Collection<Entity>
				     gold, Collection<Entity> guess)
    {
	//--Relevant Mentions
	Map<Integer,Pair<Integer,Pair<Mention,Entity>>> mentionLocationsGold = new
	    HashMap<Integer,Pair<Integer,Pair<Mention,Entity>>>();
	Map<Integer,Pair<Integer,Pair<Mention,Entity>>> mentionLocationsGuess = new
	    HashMap<Integer,Pair<Integer,Pair<Mention,Entity>>>();
	for(Mention m : mentions){
	    //(if it's relevant to this sentence)
	    if(m.sentence == this){
		...
		mentionLocationsGold.put(m.beginIndexInclusive,Pair.make(m.endIndexExclusive,Pair.make(m,correspondingEntity)));
		...
		mentionLocationsGuess.put(m.beginIndexInclusive,Pair.make(m.endIndexExclusive,Pair.make(m,correspondingEntity)));
	    }
	}
	//--Format
	for(int i=0; i<words.size(); i++){
	    if(mentionLocationsGold.containsKey(i)){
		//(get info)
		Pair<Integer, Pair<Mention, Entity>> info = mentionLocationsGold.get(i);
		Pair<Integer, Pair<Mention, Entity>> guessInfo = mentionLocationsGuess.get(i);
		int endLocation = info.getFirst();
		Mention mention = info.getSecond().getFirst();
		Entity entity = info.getSecond().getSecond();
		//(print info)
		if(entity == null){
		    ...
		    entity.format(b,mention,guessInfo.getSecond().getSecond());
		    b.append(" ");
		    i = endLocation-1;
		}
	    } else {
