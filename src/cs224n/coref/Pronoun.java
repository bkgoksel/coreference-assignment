...
public enum Pronoun {
  ...
  /**
   * Returns the Pronoun instance of the String given.
   * @param cand The candidate String
   * @returns the Pronoun instance of the candidate String
   *          null if the Enum does not exist
   */
  public static Pronoun getPronoun(String cand) {
    cand = cand.toLowerCase();
    if (!isSomePronoun(cand)) {
      return null;
    }
    if (!pronounSet.contains(cand)) {
      return null;
    }
    Pronoun p = valueOrNull(cand);
    if (p == null) return null;
    return Enum.valueOf(Pronoun.class, p.name());
  }
